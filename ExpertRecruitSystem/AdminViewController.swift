...
let kkFirebaseTermsOfService = NSURL(string: "https://firebase.google.com/terms/")!
...
let kkGoogleAppClientID = (FIRApp.defaultApp()?.options.clientID)!

// Your Facebook App ID, which can be found on developers.facebook.com.
let kkFacebookAppID     = "your fb app ID here"
...
class AdminViewController: UIViewController {
    ...
    func updateUI(auth auth: FIRAuth, user: FIRUser?) {
        if let user = user {
            ...
            self.signedInLabel.text = "Signed in as Administrator"
    ...
    static func fromStoryboard(storyboard: UIStoryboard = AppDelegate.mainStoryboard ) -> AdminViewController {
        return storyboard.instantiateViewControllerWithIdentifier("AdminViewController") as! AdminViewController
    }
}
